...
class ArchiveServerJob implements ShouldBeUnique, ShouldQueue
{
    ...
    public function uniqueId(): string { return (string) $this->serverId; }
    public function handle(StartArchiveService $archive, PolicyResolver $policies): void
    {
        $server = Server::query()->findOrFail($this->serverId); $state = ServerLifecycleState::query()->where('server_id', $server->id)->firstOrFail(); $policy = $policies->resolve($state);
        if (! config('server-lifecycle.enabled') || ! $state->automatic_enabled || ! $policy || ! $state->archive_due_at?->isPast() || $state->exempt_until?->isFuture()) return;
