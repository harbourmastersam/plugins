...
class PermanentDeleteArchiveService
{
    ...
    {
        $canAdminDelete = $actor->can('delete serverArchive');
        $isOwnerAllowed = config('server-lifecycle.users_may_delete')
            && (int) $archive->owner_id === (int) $actor->id;
        abort_unless($canAdminDelete || $isOwnerAllowed, 403);

        $this->handle($archive, true, $canAdminDelete ? 'manual_admin' : 'manual_owner');
    }
    ...
    public function handle(ServerArchive $archive, bool $manualOverride = false, string $reason = 'retention_expired'): void
    {
        Cache::lock("server-lifecycle:delete:$archive->id", 300)->block(5, function () use ($archive, $manualOverride, $reason): void {
            ...
            $allowed = $manualOverride
                ? [LifecycleStatus::Archived, LifecycleStatus::ArchiveSuperseded, LifecycleStatus::DeletionWarning, LifecycleStatus::Restored, LifecycleStatus::RestoreFailed, LifecycleStatus::PendingDeletion, LifecycleStatus::DeleteFailed]
