...
When an automatically due server is positively observed in an active Wings state, that observation restarts its inactivity window. Confirmed `Missing` and transport failures never create fake activity. Initial archival still requires exact `Offline`. In the narrower post-adoption cleanup state, confirmed `Missing` is accepted because an earlier native deletion attempt may already have removed Wings; an active status instead supersedes the archive and an unreachable daemon remains fail-closed.
