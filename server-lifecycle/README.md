...
Archival obtains a distributed server lock, records the activity boundary, atomically changes the state to `ARCHIVING`, and requires the exact Wings `Offline` state. Destructive decisions obtain a fresh request from Pelican's `DaemonServerRepository::getHttpClient()` rather than Pelican's cached status helper or a raw daemon client. This preserves Pelican's authenticated node client and Wings token-identity validation. A validated Wings HTTP 404 may arrive through the native client's `RequestException`; the plugin checks that response's Wings User-Agent and exact node token ID before treating it as confirmed missing. Intermediary 404s, transport, authentication, 5xx, malformed-response, and unknown-state failures remain unsafe. Confirmed missing can authorize only the narrow post-adoption cleanup path. Initial archival always rejects missing, running, unsafe inactive, and unreachable states. It rejects attached databases and currently unsupported material metadata, captures an encrypted manifest plus a credential-free policy snapshot, and starts an asynchronous locked backup on the policy-selected host. Meaningful activity invalidates the attempt. Immediately before adoption the plugin reloads the state and server, checks the same attempt and activity boundary, and performs two uncached exact-Offline reads around a final state refresh. On verified completion, `AdoptBackupAsArchiveService` deletes only the temporary **database row** (never via Pelican's remote-delete service), refreshes the Server, and calls Pelican `ServerDeletionService`. If deletion fails, the adopted object remains and a backoff-controlled reconciliation job retries only native server deletion; duplication is preferred to loss.
...
* Existing Pelican backup history blocks lifecycle archival. Version 1 preserves only its final lifecycle archive and will not silently destroy historical Backup records during native server deletion.
* Servers with subusers, schedules/tasks, mounts, or server-scoped webhook configurations are blocked rather than silently losing material Panel-side configuration. Global webhook configurations do not block an individual server.
