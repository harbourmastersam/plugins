<x-filament-panels::page>
    ...
    >
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-gray-900">
            {{-- Profile identities and mapping rows share the same columns, creating one continuous workspace. --}}
            <div class="grid lg:grid-cols-[minmax(0,1.25fr)_5rem_minmax(0,1fr)]">
                <div class="p-5 sm:p-6">
                    <div class="flex items-start gap-4">
                        <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg border border-gray-200 bg-gray-50 text-gray-500 dark:border-white/10 dark:bg-white/5 dark:text-gray-400">
                            <x-filament::icon icon="tabler-shield-lock" class="h-6 w-6" />
                        </div>
                        <div class="min-w-0 flex-1">
                            <p class="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Source profile</p>
                            <h2 class="mt-1 font-semibold text-gray-950 dark:text-white">Identity Provider Profile</h2>
                            ...
                            <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">OAuth / OIDC claims · dot notation supported</p>
                        </div>
                    </div>
                </div>

                <div class="flex items-center justify-center border-y border-gray-200 py-2 text-gray-400 dark:border-white/10 dark:text-gray-500 lg:border-y-0 lg:py-0" aria-hidden="true">
                ...
                <div class="bg-gray-50 p-5 dark:bg-white/5 sm:p-6">
                    <div class="flex items-start gap-4">
                        <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg border border-gray-200 bg-white text-primary-600 shadow-sm dark:border-white/10 dark:bg-gray-900 dark:text-primary-400">
                            <x-filament::icon icon="tabler-user-cog" class="h-6 w-6" />
                        </div>
                        <div class="min-w-0">
                            <p class="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Target profile</p>
                            <h2 class="mt-1 font-semibold text-gray-950 dark:text-white">Pelican User Profile</h2>
                            <p class="mt-3 text-sm font-medium text-gray-700 dark:text-gray-200">Pelican</p>
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">User and installed plugin attributes</p>
                        </div>
                    </div>
                </div>
            </div>

            <div class="hidden grid-cols-[minmax(0,1.25fr)_5rem_minmax(0,1fr)] border-y border-gray-200 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:border-white/10 dark:text-gray-400 lg:grid">
                <span class="px-6 py-3">Identity provider attributes</span>
                <span class="py-3 text-center"><span class="sr-only">Mapping direction</span></span>
                <span class="bg-gray-50 px-6 py-3 dark:bg-white/5">Pelican attributes</span>
            </div>

            @forelse ($groups as $group => $rows)
                <section aria-labelledby="mapping-group-{{ \Illuminate\Support\Str::slug($group) }}" class="border-b border-gray-200 last:border-b-0 dark:border-white/10">
                    <div class="flex items-center gap-4 bg-gray-50/60 px-4 py-3 dark:bg-white/[0.025] sm:px-6">
                        ...
                        @foreach ($rows as $rowIndex => $row)
                            <div wire:key="target-{{ $row['key'] }}">
                                @foreach ($row['mappings'] as $mappingIndex => $mapping)
                                    ...
                                    @endphp
                                    <div
                                        class="grid lg:grid-cols-[minmax(0,1.25fr)_5rem_minmax(0,1fr)] lg:items-stretch {{ $mappingIndex > 0 ? 'border-t border-dashed border-gray-200 dark:border-white/10' : '' }}"
                                    ...
                                    >
                                        ...
                                        <div class="flex min-w-0 items-center justify-between gap-3 bg-gray-50 px-4 py-4 dark:bg-white/5 sm:px-6">
